...
Int_t length_names = sizeof(_names)/sizeof(*_names);
void genMacro()
{
	...
	//overplot(___layerHistosA, ___layerHistosA, _tlk, nEntries, 1);
}
...
void getRingLayerHistos(TDirectoryFile *_nTuple, TH1F ****____layerHistos,Int_t nEntries,Int_t bias)
{
	...
	(*____layerHistos) = (TH1F ***) malloc((length_names)*sizeof(TH1F **));
	for (iTree=0;iTree<nTrees-bias;iTree++)
	{
		(*____layerHistos)[iTree] = (TH1F **) malloc(nLayers*sizeof(TH1F *));
	}
	_tlk = _nTuple->GetListOfKeys();
	// Get histograms from TTrees
	//for (iTree=0;iTree<nTrees-bias;iTree++)
	for (iTree=0;iTree<length_names;iTree++)
	{
		//cout << "iTree" << iTree << " _tlk name -> " << _tlk->At(iTree+bias)->GetName() << endl;
		cout << _names[iTree] << endl;

		
		_nTuple->GetObject(_tlk->At(iTree+bias)->GetName(),_tree);
		...
		for (iLayer=0;iLayer<nLayers;iLayer++)
		{
			...
			cout << _nTupleTree->GetRingLayerHisto(iLayer) << endl;
			//(*____layerHistos)[iTree][iLayer] = _nTupleTree->GetRingLayerHisto(iLayer);
			//cout << ____layerHistos[iTree][iLayer] << endl;
			//if (____layerHistos[iTree][iLayer] == NULL)
			if (_nTupleTree->GetRingLayerHisto(iLayer) == NULL)
			{
				cout << "Is NULL" << endl;
			}
